...
//имя .csv  файла с показаниями датчика температуры
	...
	//массив структур  с даными о показаниях температуры за год
	static struct temperature_sensor data[SIZE];
        ...
        else
           {
			 ...
			 else
			{
				...
				// printf("%u :%u %u %u %u %u %d\n",line_number,year,month,day,hour,minute,temperature); 
				 ...
				 AddInfo(data,count,year,month, day, hour, minute, temperature);
				 count++;
